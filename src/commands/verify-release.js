...
// `verify-release` command — interactive post-release verification checklist
...
export const verifyReleaseCommand: any = new Command("verify-release")
  ...
  .action(async (options: any) => {
    ...
    ui.header(`Verify Release — ${version}`);
    ui.docRef(DOCS.verifyRelease);
    ...
    ui.dim(`     npx @react-native-community/cli init TestApp --directory /tmp/TestApp --version ${version}`);
    ...
    console.log();
    {
      ...
      if (action === "run") {
        ...
        const cmd = `npx @react-native-community/cli init TestApp --directory /tmp/TestApp --version ${version}`;
        ui.info("  Running: " + cmd);
    ...
    console.log();
    ui.success("✅ Verification checklist complete!");
  });
